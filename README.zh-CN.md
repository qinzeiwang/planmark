...
助手会保留原始空间关系，使用[原提示词](base-map/prompts/redraw.md)。默认采用[浅色参考图](base-map/references/default.png)，需要更强手绘感时采用[备选图](base-map/references/hand-drawn.png)。每次只用一张风格图，不能把参考图的建筑布局复制到用户地图上。
...
- **重启服务**：关闭服务后，告诉 Codex 独立项目目录，让它重新启动该目录内的 `scripts/local-server.py` 并打开 `editor/start.html`。先保存 JSON；已保存的改动需重新导入，初始 start.html 不会自动更新成最后编辑状态。

详细差异见[保存与导出说明](export/guide.md)。
